 ...
 *         move upward. For legibility in this example, these offset values will
 ...
 *         2 (3) 0 1 -3 - step forward
 *         because of the instruction we just modified. The first instruction is
 *         incremented again, now to 2. 
 *         2 4 0 1 (-3) - jump all the way to the
 *         end; leave a 4 behind. 
 *         2 (4) 0 1 -2 - go back to where we just were;
 *         increment -3 to -2. 
 *         2 5 0 1 -2 - jump 4 steps forward, escaping the
 *         maze. In this example, the exit is reached in 5 steps.
 * 
...
public class AdventOfCodeDay5 {
	...
	private static void part1(int[] input) {
		...
		int jumpToIndex = 0;
		int steps = 0;
		while (jumpToIndex >= 0 && jumpToIndex < input.length) {
			jumpToIndex = index + input[index];
			input[index]++;
			index = jumpToIndex;
			steps++;
		}
