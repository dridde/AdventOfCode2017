 ...
 *         in your direction. "You there! Your state appears to be idle. Come
 ...
 *         and 3, and their difference is 4. The third row's difference is 6. In
 *         this example, the spreadsheet's checksum would be 8 + 4 + 6 = 18.
 * 
 ...
 *         second row, the two numbers are 9 and 3; the result is 3. In the
 *         third row, the result is 2. In this example, the sum of the results
 *         would be 4 + 3 + 2 = 9.
 * 
...
public class AdventOfCodeDay2 {
	...
	public static void main(String[] args) {
		int[][] input = { { 179, 2358, 5197, 867, 163, 4418, 3135, 5049, 187, 166, 4682, 5080, 5541, 172, 4294, 1397 },
		...
		for (int i = 0; i < input.length; i++){
			...
			for (int j = 0; j < input[i].length; j++){
				//part 1 stuff: find min & max per row
				...
				for (int k = 0; k < input[i].length; k++){
					...
					if (input[i][j] % input[i][k] == 0) {
						//sum for modulo = 0 numbers
						checksum2 += input[i][j] / input[i][k];
						break;
					}
				}
			}
			//part 1 stuff: sum min max difference per row
			checksum += max - min;
		}
