 ...
 * from the third word can be rearranged to form the first word. a ab abc abd
 * abf abj is a valid passphrase, because all letters need to be used when
 * forming another word. iiii oiii ooii oooi oooo is valid. oiii ioii iioi iiio
 * is not valid - any of these words can be rearranged to form any other word.
...
public class AdventOfCodeDay4 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i< input.length; i++) {
			String passphrase = input[i];
			if (checkValidity(passphrase)) {
				valid++;
			}
			if (checkValidity2(passphrase)) {
				valid2++;
			}
		}
	 ...
	 * @return
	 */
	private static boolean checkValidity(String passphrase) {
		String[] split = passphrase.split(" ");
		for (int j = 0;j<split.length;j++) {
			for (int k = 0; k< split.length;k++) {
				if (j!=k && split[j].equals(split[k])) {
					return false;
				}
			}
		}
	...
	/**
	 * checks if passphrase is valid according to rule 2
	 * @param passphrase
	 * @return
	 */
	private static boolean checkValidity2(String passphrase) {
		String[] split = passphrase.split(" ");
		for (int j = 0;j<split.length;j++) {
			for (int k = 0; k< split.length;k++) {
				if (j!=k){
					char[] word1 = split[j].toCharArray();
					char[] word2 = split[k].toCharArray();
					Arrays.sort(word1);
					Arrays.sort(word2);
